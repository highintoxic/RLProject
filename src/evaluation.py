...
def _get_judge_client():
    """Create OpenRouter client for LLM judge."""
...
def llm_judge(
    ...
    Args:
        ...
        facts: Case facts.
        reference: Ground truth judgment.
        prediction: AI-generated judgment.
        judge_model: Override judge model (default: config CHAT_MODEL).

    Returns:
        dict with scores for legal_accuracy, reasoning_chain, etc.
    """
    model_id = judge_model or CHAT_MODEL
...
def run_llm_judge_eval(
    ...
    for i, case in enumerate(dataset.select(range(n_samples))):
        ...
        prediction = generate_judgment(model, tokenizer, facts)
        scores     = llm_judge(
            client, facts, reference, prediction,
